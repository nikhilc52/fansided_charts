...
data |> 
  ggplot(aes(x=Gcar))+
  geom_line(aes(y = rolling_fgp, color = "#007ac1"), size = 1.5, alpha=1) +
  geom_line(aes(y = rolling_tpp, color = "#ef3b24"), size = 1.5, alpha=1) +
  geom_line(aes(y = rolling_tsp, color = "#fdbb30"), size = 1.5, alpha=1) +
  scale_color_identity()+
  labs(
    title="SGA's shooting efficiency for the last 3 seasons",
