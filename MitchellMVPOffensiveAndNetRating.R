...
off <- read.csv("C:\\Users\\nikhi\\Downloads\\OffCombinedData.csv")
on <- read.csv("C:\\Users\\nikhi\\Downloads\\OnCombinedData.csv")
...
plotOffOn <- on |> 
  ...
  mutate(team_alpha = ifelse(VS_PLAYER_NAME.x == "Mitchell, Donovan", 0.9, 0.01)) |> 
...
plotOffOn |> 
  ggplot(aes(x=OFF_RATING.x, y=OFF_RATING.y)) +
  ...
  ylab('Offensive Rating Off-Court') + 
  xlab('Offensive Rating On-Court') +
  labs(title="Individual On/Off Offensive Rating", 
       subtitle = "Min. 40 GP, 1200 Min, Size = # Minutes", 
       caption="Twitter/X: @nikhil_chinchal\nData: NBA_API")+
  theme(plot.title = element_text(size = 22, hjust =0.5, face = "bold"), 
        plot.subtitle = element_text(size = 16, hjust =0.5))+
  geom_hline(yintercept = mean(plotOffOn$OFF_RATING.y), linetype="dashed")+
  geom_vline(xintercept = mean(plotOffOn$OFF_RATING.x), linetype="dashed")+
  geom_text_repel(aes(label = ifelse(VS_PLAYER_NAME.x == "Mitchell, Donovan", "Donovan Mitchell", "")),
                  position = position_nudge_repel(x = 1, y = 1), 
                  max.overlaps = Inf)
...
plotOffOn |> 
  ggplot(aes(x=NET_RATING.x, y=NET_RATING.y)) +
  geom_point(aes(fill=team_color, color=team_color, alpha = team_alpha, size = MIN.x),
             shape=20, show.legend = FALSE) +
  scale_color_identity(aesthetics = c("fill","color"))+
  scale_x_continuous(breaks=scales::pretty_breaks(n=8))+
  ...
  theme_fivethirtyeight()+  
  theme(axis.title = element_text()) + 
  ylab('Net Rating Off-Court') + 
  xlab('Net Rating On-Court') +
  labs(title="Individual On/Off Net Rating", 
       subtitle = "Min. 40 GP, 1200 Min, Size = # Minutes", 
       caption="Twitter/X: @nikhil_chinchal\nData: NBA_API")+
  theme(plot.title = element_text(size = 22, hjust =0.5, face = "bold"), 
        plot.subtitle = element_text(size = 16, hjust =0.5))+
  geom_hline(yintercept = mean(plotOffOn$NET_RATING.y), linetype="dashed")+
  geom_vline(xintercept = mean(plotOffOn$NET_RATING.x), linetype="dashed")+
  geom_text_repel(aes(label = ifelse(VS_PLAYER_NAME.x == "Mitchell, Donovan", "Donovan Mitchell", "")),
                  position = position_nudge_repel(x = 6, y = 1), 
                  max.overlaps = Inf)
